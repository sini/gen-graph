...
let
  ...
  reachableFrom = { edges, ... }: startId:
    ...
    in builtins.filter (id: id != startId) (map (r: r.key) result);
  ...
  canReach = { edges, ... }: fromId: toId:
    builtins.any (r: r.key == toId) (builtins.genericClosure {
  ...
  selfReachable = { edges, ... }: id:
    builtins.any (r: r.key == id) (builtins.genericClosure {
...
in
{
  inherit reachableFrom reachableWhere canReach selfReachable ancestorsOf pathsBetween;
}
