...
# rebuilds the unconsumed residue and re-sorts it. ★ The absence of a mutable heap was once
# also the reason the partition gave for having no Tarjan arm. It was not a bound there either:
# Tarjan's DFS is the `lowlink` arm (`lib/partition.nix`), its stack and index map persistent,
