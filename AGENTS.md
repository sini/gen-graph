...
| ORDERING ON A DENSE GRAPH IS NOT IMPROVED BY THIS, and the shape to check it on is `total` | `--argstr shape total --arg n 2000` (node i depends on every j > i; `E = n(n-1)/2 = 1,999,000`). `list.elements` **24,082,640 = 12.05 per edge**, `sets.elements` **21,960,244**, against the same fixture on the pre-remedy library at 24,018,002 / 22,011,489 — i.e. **+0.27% list, −0.23% sets, within noise of unchanged**. Both accumulators are already small here; what dominates is the REVERSE INDEX, measured at 4.00 per edge and 36.3% of the total, which neither remedy touches. ★ The follow-ups spec's O2 dense budget (≤ 7.0 per edge, ≤ 14,000,000 sets) is a **rank** budget and this **FAILS** it — as does the pre-remedy library on the same fixture, at 12.01 per edge. Do not read "near-linear" as a claim about dense inputs |
...
| Both cost shapes in the bench were CYCLIC, so the emission loop ran zero steps in every cell | `complete` and `cycle` exist to price the cycle path, so every node has indegree ≥ 1 and the initial ready set is empty. `nix-instantiate --eval --strict --arg n 200 --argstr arm topoOrder --argstr shape <s> ./ci/bench/cost-classes.nix` ⇒ `initialReady` = 0 (`complete`), 0 (`cycle`), 1 (`chain`), 100 (`wide`), 20 (`fleet`), 100 (`discrim`), and 2001 at `n=8000` (`deepwide`). The control is read on every run: a cell reporting 0 says nothing about ordering cost |
