...
| The remaining bound on ordering is COST, not frames, and it is quadratic | `nix-instantiate --eval --strict --arg n <n> --argstr arm topoOrder --argstr shape <shape> ./ci/bench/cost-classes.nix` under `NIX_SHOW_STATS`. `list.elements`, `shape=chain`, `n = 500/1000/2000/4000/8000` ⇒ 136,744 / 523,494 / 2,046,994 / 8,093,994 / 32,187,994 — ×3.98 per doubling, exponent **2.00**; `shape=wide`, same sizes ⇒ 134,752 / 519,502 / 2,039,002 / 8,078,002 / 32,156,002, same exponent. Floor in the same runs (`arm=floor`, no gen-graph code): `6n + 3` on both shapes — 6,003 at `n=1000`, 48,003 at `n=8000` — exponent 1.00. These are POST-CHANGE cells, which the in-repo bench does produce. ★ Before quoting a `sets.elements` figure from it, run `./ci/bench/sentinel.sh` — `sets` figures are comparable only within one revision of that file |
| The quadratic is TWO accumulators, and the ready set is no longer one of them | `emitted = st.emitted ++ [ pick ]` allocates a fresh k-vector at step k and `st.indeg // …` copies the indegree map per step; both Θ(n²). The ready set is a leftist heap, Θ(n log n). Held instead as a sorted array rebuilt and re-sorted on every arrival it was a THIRD independent quadratic: measured share of `list.elements` at `n=8000` ⇒ `wide` 59.89%, `tree` 42.72%, `discrim` 42.75%, `fleet` 21.17%, `chain` 0.02%. ★ PRODUCER: a share is a TWO-REVISION comparison and the array arm exists in no arm of `ci/bench/cost-classes.nix`, so no command in this repo yields one — `den-architecture` `specs/2026-08-08-gen-graph-ready-set-quadratic.r1-*` (`r1-data-sweep.csv`, arms `real`@base vs `real`@impl; procedure in `.r1-data-note.md`). ★ The chain is a DEAD PREDICATE for it — its ready set never holds two nodes — so a chain-only reading measures the term at n−1 elements and its share FALLS with n (0.19% → 0.02% over n = 1000 → 8000). Any cost claim about the ready set needs `shape=wide` or `deepwide` |
| The heap's price is on the SET axis, and it is stated rather than netted | `sets.elements`, `shape=wide`: the heap adds one attrset per node on each merge path, +63.49 / +71.90 / +80.15 / +88.67 per node at `n = 1000/2000/4000/8000` — growing like log n, i.e. Θ(n log n). PRODUCER: same two-revision harness, `r1-data-sweep.csv`, `sets.elements` on `real`@impl minus `real`@base. Today it is dominated by the indegree map's Θ(n²) and invisible in the exponent; it becomes the leading set-axis term once that map is fixed. A single-axis reading of this change would report a 60% improvement and miss the addition entirely |
