...
| Trap | Evidence |
|---|---|
| `phaseOrder` does **not** use `prelude.toposort` — it goes through `condensation` → `transitiveClosure` | `lib/order.nix:14,42-45`; `grep -rho "prelude\.[a-zA-Z']*" lib/ \| sort -u` ⇒ exactly `concatMap filterAttrs fix foldl' genAttrs listToAttrs mapAttrs max unique`, no `toposort`. Positive control on the same predicate: gen-prelude **does** export it — filtering the pinned prelude's `attrNames` for `topo\|sort` ⇒ `["sort","toposort"]` |
