...
| A `sets.elements` figure from `ci/bench/cost-classes.nix` is quotable only with the SENTINEL beside it | `./ci/bench/sentinel.sh` ⇒ `verdict = "STABLE"` at this rev. The file is a mutable instrument: adding the ordering cost class shifted `sets.elements` +1 on all twelve pre-existing arm × shape cells while `list.elements` stayed bit-identical on every one. `SHIFTED-BENIGN` ⇒ re-run every cell on the frozen file, re-pin, publish WITH THE OFFSET LEFT IN — never subtract it, which makes the figure irreproducible from the stated command. `UNUSABLE` never collapses to `STABLE`. ★ Its operating assumption is ONE CHANGE AT A TIME: two benign changes on different evaluation paths sum to a non-uniform delta and read `SHIFTED-STRUCTURAL` (measured — a bench edit at +3 everywhere plus a prelude bump at +6 where the prelude is reachable gave +9/+3 across 120 cells, with `list` and `nrLookups` bit-identical throughout). Treat that verdict as a prompt to decompose |
