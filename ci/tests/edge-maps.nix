{ lib, graphLib, ... }:
let
  inherit (graphLib) materialize materializeParents unionEdges intersectEdges differenceEdges selectEdges;
  inherit (graphLib.mock) fixtures mkGraph;
in
{
  edge-maps = {
    test-materialize-chain = {
      expr = let mat = materialize fixtures.chain;
      in mat."a";
      expected = [ "b" ];
    };
    ...
    test-materialize-diamond = {
      expr = let mat = materialize fixtures.diamond;
      in builtins.sort builtins.lessThan mat."a";
      expected = [ "b" "c" ];
    };
    test-union-dedup = {
      expr = let
      ...
      in builtins.sort builtins.lessThan result.x;
      expected = [ "a" "b" "c" ];
    };
    test-union-disjoint = {
      expr = let
    ...
    test-intersect = {
      expr = let
        a = { x = ["a" "b" "c"]; };
        b = { x = ["b" "c" "d"]; };
      in builtins.sort builtins.lessThan (intersectEdges a b).x;
      expected = [ "b" "c" ];
    };
    ...
    test-difference = {
      expr = let
        a = { x = ["a" "b" "c"]; };
        b = { x = ["b"]; };
      in builtins.sort builtins.lessThan (differenceEdges a b).x;
      expected = [ "a" "c" ];
    };
    ...
    test-materialize-parents-tree = {
      ...
      in mp;
      expected = { child1 = "root"; child2 = "root"; grandchild = "child1"; };
    };
    ...
    test-select-edges-basic = {
      ...
      in builtins.sort builtins.lessThan (filtered."api" or []);
      expected = [ "cache" "db" ];
    };
    test-select-edges-by-target = {
      ...
      in builtins.sort builtins.lessThan (builtins.attrNames filtered);
      expected = [ "api" "worker" ];
    };
