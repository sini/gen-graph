...
let
  ...
  standalone = import ../.. { prelude = genPrelude; };
in
{
  ...
  flake.tests.entry.test-control-the-compared-surface-is-non-trivial = {
    expr = builtins.length (builtins.attrNames standalone) > 40;
    expected = true;
  };
  ...
  flake.tests.entry.test-control-the-comparison-discriminates = {
    expr = builtins.attrNames standalone == builtins.attrNames (removeAttrs genGraph [ "roots" ]);
    expected = false;
  };
  ...
  # runs a real traversal over the shipped diamond fixture, so the accessor machinery and the
  # prelude combinators it folds with both have to be the real ones.
  flake.tests.entry.test-the-shims-library-is-live = {
    expr = standalone.roots standalone.fixtures.diamond;
    expected = [ "a" ];
  };
