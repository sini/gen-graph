...
# documents, and every OTHER cell in this suite takes `genGraph` from `ci/flake.nix`, which builds it
...
# resolver-seam shape below: one dependency, `prelude`, wired the same three-channel way every other
# library in this roster now is.
#
# ★ THIS FILE'S FORMAL IS `genPrelude`, NOT `prelude` — `ci/flake.nix`'s `specialArgs` supplies it
# under that name (`genPrelude = prelude;`), the harness's own naming for the injected library
# instance. The SHIM's own dependency formal is `prelude`; `entryArgs` below binds the harness's
# `genPrelude` value to that shim formal name, which is the one place the two names meet.
#
...
#   prelude — `roots`' target-set fold runs through `prelude.concatMap` over the node list, so
#             forcing its result is what drives the call through gen-prelude rather than merely
#             returning the caller-supplied node list unread.
#
...
in
{
  ...
  flake.tests.entry.test-the-defaulted-entry-publishes-the-flake-surface =
    ...
    in
    {
      ...
      expected = builtins.attrNames genGraph;
    };
