...
| `cyclicEdgesWhere`                                  | `Θ(Σ_v (\|reach⁺ v\| + \|reach⁻ v\|))` over the endpoints of the edges satisfying `p`, through the `fbNode` partition arm, plus one pass over the labelled edges — QUADRATIC on a ring and on an acyclic chain, linear on a star. `nrFunctionCalls` with `p` accepting every edge, n = 250 → 2000: ring 650,249 → 40,202,423 (×3.98 per doubling), chain 20,162,012 at n = 2000 (×3.97), star 233,465 (×2.13); `p` always false: 16,031 on the ring (×2.00)                                                                                                                                                                                                                                                                                                                                                                            | the tags are lazy and the `p` test short-circuits, so only an endpoint of a p-satisfying edge pays its two closures. `fbWork` would not remove the quadratic — linear on the ring, quadratic on a chain listed tail-first. A linear, non-recursive SCC construction is in progress                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
