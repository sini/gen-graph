...
**`condensation g`** — collapses each SCC to a super-node and returns the condensation (quotient) graph. This is the **one published front door for SCC partitioning**; it carries the default, and the arms behind it are published under their own names (see *The partition routing contract* below). Not Tarjan's linear single-DFS — not for want of a mutable stack, which a persistent structure supplies, but because that pass is a **self-applying DFS step**: ADR-0022 constrains SCC detection to be non-recursive, and the evaluator's call-depth ceiling ends a self-applying step with an uncatchable `max-call-depth exceeded`. Both arms iterate over a worklist instead. Returns a record:
