...
**`condensation g`** — collapses each SCC to a super-node and returns the condensation (quotient) graph. This is the **one published front door for SCC partitioning**; it carries the default, and the arms behind it are published under their own names (see *The partition routing contract* below). Not Tarjan's linear single-DFS — not for want of a mutable stack, which a persistent structure supplies, but because that pass is a **self-applying DFS step**: SCC detection here is constrained to be non-recursive, since the evaluator's call-depth ceiling ends a self-applying step with an uncatchable `max-call-depth exceeded`. Both arms iterate over a worklist instead. Returns a record:
...
| `cyclicEdgesWhere`                                  | `Θ(Σ_v (\|reach⁺ v\| + \|reach⁻ v\|))` over the endpoints of the edges satisfying `p`, through the `fbNode` partition arm, plus one pass over the labelled edges — QUADRATIC on a ring and on an acyclic chain, linear on a star. `nrFunctionCalls` with `p` accepting every edge, n = 250 → 2000: ring 650,249 → 40,202,423 (×3.98 per doubling), chain 20,162,012 at n = 2000 (×3.97), star 233,465 (×2.13); `p` always false: 16,031 on the ring (×2.00)                                                                                                                                                                                                                                                                                                                                                                            | the tags are lazy and the `p` test short-circuits, so only an endpoint of a p-satisfying edge pays its two closures. `fbWork` would not remove the quadratic — linear on the ring, quadratic on a chain listed tail-first. A linear, non-recursive SCC construction is the open spike den-hoag-c48r1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
