...
★★ **THE ABSENCE OF A REFUSAL IS NOT EVIDENCE THAT YOUR `step` IS MONOTONE, and the gap is the same defect class.** Support is strictly weaker than foundedness: a conclusion drawn on an absence that later acquires a **circular** re-derivation is supported, and is returned. `p :- not r. p :- p. r :- a` has a non-monotone step (measured: `step {a}` yields `p`, `step {a,r}` does not), is **not** refused, and returns `p` — which the well-founded model makes FALSE. Closing that is ADR-0020's well-founded engine, which that ADR puts in Phase-C den territory. The ceiling is instrumented, not just described: `test-seeded-circular-rederivation-is-supported-and-RETURNED`.

Measured price: one extra full step, and **the axis is ROUND COUNT, not size**. On the canonical closure instance over a 400-node chain (~399 rounds) it is **+0.73%** (45,307,630 → 45,637,203 thunks), `cpuTime` ranges overlapping either side — but on a **diameter-2 graph, which converges in 2 rounds, it is +31% to +36%**, stable at 402 and at 1,602 nodes. Shallow dependency graphs pay the high end.
...
| `seededFixpoint` | O(work per delta), **plus one full step at convergence** | semi-naive: each iteration touches only the frontier. The support check re-derives once from the converged accumulator, and **what dilutes it is ROUND COUNT, not size** — the loop's per-round unions and differences dominate only when there are many rounds. Deep instance, 400-node chain at ~399 rounds: **45,307,630 → 45,637,203** thunks, **+0.73%**; `cpuTime` 25.3–29.2s against 28.2–30.2s over three runs a side, which OVERLAPS, so the thunk count resolves it and the wall clock does not. **Shallow instance, diameter-2 at 2 rounds: +31% to +36%**, and the ratio holds across a 4× size change (402 and 1,602 nodes), which is what identifies the axis. A caller on a shallow dependency graph pays the high end |
