...
lowlink            : { edges, nodes, ... } → <the same record>   # partition arm, by name; a closed accessor only
...
**What the result carries.** The partition itself (`sccs`, `sccOf`, `condEdges`, plus `reps`/`bottomUp`/`members`) and one measure, `depth`. All plain data. Every arm returns the identical record on every graph where all of them answer, and that is a property of the CONSTRUCTION rather than of the cells that check it: each arm computes a tag map and nothing else, then calls the one exported finisher, `condensationOf`. An arm that finished its own record would agree with its siblings only for as long as someone kept three copies in step. `condensationOf` takes an accessor and a tag map; its precondition is that the tag map is an SCC partition's, total on `nodes`, since any other map can induce a cyclic quotient — which the ordering pass refuses by name rather than answering.

**Which accessors the partition takes.** The partition-consuming surfaces take a **closed** accessor only: every target `edges` returns is in `nodes`. An edge to a node outside `nodes` means `(nodes, edges)` is not a graph, so it has no SCC partition to return, and `lowlink` refuses it by name — `gen-graph.lowlink: edges "<id>" names a target outside `nodes\`\` — as the ordering surfaces already refuse a dangling target. `cyclicEdgesWhere` and `cyclePaths` bind `lowlink`, so they refuse the same input under its name, **including where the off-node target lies on no cycle and no `p`-edge**, which they used to answer. The forward–backward and closure arms still answer on an open accessor and are not guaranteed to agree with each other there. `cycles` and the traversals keep the open convention stated under *Traversal (lazy)* (a target outside `nodes` still expands through the accessor). A `nodes` entry that is not a string is refused by name by `lowlink` as well.

**When each arm wins — measured, with no budget attached.** The arms are **complementary, not ranked**: the two forward–backward arms accept the same input, `lowlink` takes a closed accessor only, and none is uniformly cheaper. Growth exponents by doubling pair, `ci/bench/cost-classes.nix`, arms `fbNode` / `fbWork` / `lowlink`:
...
| `cyclePaths`                                        | on a DAG, exactly the `cycles` cost above — so Θ(n²) on a complete DAG; + the `lowlink` partition arm (see its own row) and simple-path search once cyclic. The `cycles` term keeps it QUADRATIC in the size of one large component: `nrFunctionCalls` on `cycle` 3,442,112 → 12,884,112 at n = 1000 → 2000 (×3.74), of which `cycles` is 3,017,018 → 12,034,018; bound to `fbNode` it read 13,066,056 → 52,132,056                                                                                                                                                                                                                                                                                                                                                                                                                    | short-circuits before any path work when acyclic. Builds no closure of its own, so it inherits both callees' hoist and nothing else: measured 1.95× fewer attrsets on `cycle` at n = 400, arms `cyclePaths` / `cyclePathsUnhoisted`. **The cyclic branch is not priced by those arms** — it runs `pathsBetween`, which enumerates every simple path, and no arm here quantifies it on a dense component                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `lowlink`                                           | Θ((n + m) · log₈ n) on every shape, the log being its persistent trie's. `nrFunctionCalls` n = 1000 → 20000: `cycle` 428,170 → 10,200,182, `chain` 544,031 → 12,520,041, `rand` 490,170 → 11,960,771 — span exponents 1.06 / 1.05 / 1.07, and flat as calls / (n · ⌈log₈ n⌉): 107 at n = 1000–4000 and 102 at 8000–20000 on `cycle`. `list.elements` span exponents 1.07 / 1.07 / 1.08. `nrOpUpdateValuesCopied` flat at 127 on `cycle` and `chain`; on `rand` it rises with the component count (127 → 2,177), and that is the shared finisher's term, not the arm's: `fbNode` reads the identical 1,055 and 2,141 at n = 2000 and 4000                                                                                                                                                                                               | one `genericClosure` loop, one step per push, edge, finished frame or popped member; every record the loop carries is built by a constructor that forces each field                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
