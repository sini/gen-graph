...
occurrence wins) via a threaded visited set. They visit only what they reach, and a
...
therefore no longer independent of `acc`: an `acc` that throws refuses the whole call, including a
caller who reads `.visited` alone, and with two bad inputs the refusal reported is the one at the
earlier frame. The seed `acc` is forced even when `roots = [ ]`. An `acc` whose own field is a
lazily chained list (`{ xs = a.xs ++ [ f ]; }`) keeps that chain's ceiling where you read it: it
is the caller's, not the library's.

**Price — an infinite graph diverges.** An unbounded stream of distinct keys, or `null`-keyed
...
**`cycles g`** — nodes that appear in any cycle (self-reachable). Read off the SCC partition: a node lies on a cycle iff its component has two or more members or it has an edge to itself (Tarjan 1972, Lemma 9). It binds the `lowlink` arm by name, so it costs Θ((n + m) · log₈ n) and takes a **closed** accessor only (*Which accessors the partition takes*). Returns a sorted list.
...
The walk for a component C with smallest member `u` is the one a depth-first search finds: take `u`'s first successor `v` inside C, then search from `v` for `u`, successors in `edges` order, never visiting a node twice, restricted to C. That is the first simple path from `v` to `u` in that order — the one `pathsBetween` enumerates first, which this surface used to read off it — because a node the search has finished without reaching `u` reaches `u` only through the search's current stack. The search is iterated like `lowlink` (one `genericClosure` loop over a persistent trie), so the whole surface is Θ((n + m) · log₈ n) on every graph, cyclic or not, and has no depth cap: it no longer inherits `pathsBetween`'s. It takes a **closed** accessor only.
...
**`condensation g`** — collapses each SCC to a super-node and returns the condensation (quotient) graph. This is the **one published front door for SCC partitioning**; it carries the default, and the arms behind it are published under their own names (see *The partition routing contract* below). Tarjan's single DFS is one of those arms, **`lowlink`**. SCC detection here is constrained to be non-recursive, since the evaluator's call-depth ceiling ends a self-applying step with an uncatchable `max-call-depth exceeded`, so the DFS is not written as a recursion: its call stack is data, stepped one edge or one pop at a time by a single `genericClosure` loop over a persistent 8-ary trie. That costs Θ((n + m) · log₈ n), the log being the persistent map's — not Tarjan's O(V + E). The two forward–backward arms iterate over a worklist. Returns a record:
...
**Which arm answers by default.** `condensation` is the front door and delegates to **`lowlink`**, Tarjan's single DFS iterated — the arm that is Θ((n + m) · log₈ n) on every shape. It used to delegate to `fbNode`, which is quadratic in the size of one large component; the one shape where `lowlink` costs more is `fleet` (many small components), by a constant (×2.19 `nrFunctionCalls` at n = 1,000). A consumer that derived a figure from the door's cost re-derives it against the new arm: gen-scope's `verifiedDepth` (2049) is still true and now understated, and gen-scope and gen-program, which reads it, owe that re-derivation on their relock; it is not re-derived here. The delegation is an identity, not a wrapper, so the door cannot drift from the arm it names. Every consumer *inside* this library binds an arm by name rather than the door — `topoOrderKahn`'s cycle report, `cycles`, `cyclePaths` and `cyclicEdgesWhere` all bind `lowlink` — because the door's own result is finished with an ordering pass, and an ordering surface reading the door would be ordering through a surface that orders through it.

**How the other arms are reached.** By name: `fbNode`, `fbWork`, `lowlink` and `condensationClosure`. There is no mode flag on the door, and that is deliberate: a routing decision is a property of the program, computed once, and a caller-selected mode is a second place the default lives — one a caller can use to select the arm that cannot express their graph. A named arm has no default to disagree with.

**What the result carries.** The partition itself (`sccs`, `sccOf`, `condEdges`, plus `reps`/`bottomUp`/`members`) and one measure, `depth`. All plain data. Every arm returns the identical record on every graph (every closed accessor; the rest are refused, *Which accessors the partition takes*), and that is a property of the CONSTRUCTION rather than of the cells that check it: each arm computes a tag map and nothing else, then calls the one exported finisher, `condensationOf`. An arm that finished its own record would agree with its siblings only for as long as someone kept three copies in step. `condensationOf` takes an accessor and a tag map; its precondition is that the tag map is an SCC partition's, total on `nodes`, since any other map can induce a cyclic quotient — which the ordering pass refuses by name rather than answering.

**Which accessors the partition takes.** Every surface built on the partition — the three arms, the door, `condensationClosure`, `cycles`, `cyclePaths` and `cyclicEdgesWhere` — takes a **closed** accessor only: every target `edges` returns is in `nodes`. An edge to a node outside `nodes` means `(nodes, edges)` is not a graph (Tarjan 1972 §2: E is a set of pairs of V), so it has no SCC partition to return. The domain is checked **once**, in the finisher every arm shares (`condensationOf`), so every surface refuses it by name and catchably, the way the ordering surfaces refuse a dangling target: `gen-graph.<arm>: edges "<id>" names a target outside `nodes\`\`. The name is the arm's — `fbNode`, `fbWork` or `lowlink`; `lowlink` for the door and for `cycles`, `cyclePaths` and `cyclicEdgesWhere`, which bind it; `condensationOf` for `condensationClosure`, which reaches the finisher by that name. The refusal holds **including where the off-node target lies on no cycle**, and it is attached to the whole record, so reading any one field (`sccOf` included) refuses. A `nodes` entry that is not a string is refused by name before any target is read (`gen-graph.<arm>: got int, expected a node identifier (a string)`). The line is **node-set-total versus seeded**, not partition versus not: a surface that returns a structure over `nodes` needs a graph on `nodes`, while a seeded walk (`reachableFrom`, `canReach`, `pathsBetween`) names no domain to close. `cycles` sits on that line — it enumerates `nodes`, and its answer is a per-node reachability predicate — and it is on the closed side because it is read off the partition, and because answering where `cyclePaths` (the same question plus a witness) refuses would state two domains for one family. The traversals keep the open convention stated under *Traversal (lazy)* (a target outside `nodes` still expands through the accessor).

**When each arm wins — measured, with no budget attached.** The arms are **complementary, not ranked**: all three take the same input, a closed accessor, and none is uniformly cheaper. Growth exponents by doubling pair, `ci/bench/cost-classes.nix`, arms `fbNode` / `fbWork` / `lowlink`:

| shape                   | pairing     | `fbNode` list / sets / nrLookups | `fbWork` list / sets / nrLookups | `lowlink` list / sets / nrLookups |
| ----------------------- | ----------- | -------------------------------- | -------------------------------- | --------------------------------- |
| `cycle` (ONE component) | 500→1000    | 2.00 / 1.99 / 1.99               | 1.00 / 0.96 / 0.99               | 1.37 / 0.99 / 1.21                |
| `cycle`                 | 1000→2000   | 2.00 / 2.00 / 2.00               | 1.00 / 0.98 / 1.00               | 1.00 / 0.99 / 1.00                |
| `fleet` (n components)  | 500→1000    | 1.04 / 1.00 / 1.04               | 1.05 / 1.35 / 1.04               | 1.27 / 1.00 / 1.16                |
| `fleet`                 | 1000→2000   | 1.04 / 1.01 / 1.04               | 1.05 / 1.53 / 1.04               | 1.02 / 1.01 / 1.02                |
| `chain`                 | 500→1000    | 1.97 / 1.96 / 1.96               | 1.94 / 1.85 / 1.95               | 1.27 / 0.99 / 1.15                |
| `chain`                 | 1000→2000   | 1.99 / 1.98 / 1.98               | 1.97 / 1.92 / 1.97               | 1.02 / 1.00 / 1.00                |
| `deepwide`              | 8000→16000  | 0.02 / 0.04 / 0.06               | 0.04 / 1.02 / 0.07               | 1.01 / 1.22 / 1.09                |
| `deepwide`              | 16000→32000 | 0.04 / 0.09 / 0.12               | 0.08 / 1.56 / 0.14               | 1.01 / 1.14 / 1.08                |

Read the two extremes together, because either alone is misleading. **On one large component `fbNode` is quadratic and `fbWork` is linear** — it spends a pivot per node where the worklist spends one per component, and at n = 2000 on `cycle` that is 36,048,017 list elements against 62,018. **On many small components the ranking reverses on the allocation axis**: `fbWork`'s accumulator `acc.tags // …` copies the whole growing tag map once per component, so it is **quadratic in copies whatever the node order** — `nrOpUpdateValuesCopied` 1,565,126 → 25,010,126 on `fleet` at n = 5000 → 20000 (exponent 2.00) while its calls read 1.03 — and its `sets` term grows with it (1.35 → 1.53 on `fleet`, 1.88 over 5000 → 20000, and 1.02 → 1.56 on `deepwide`), while `fbNode`, which carries no accumulator at all, stays flat at 1.00–1.01. On one large component `fbWork` is linear on every axis: `cycle` 5000 → 20000 reads calls 1.00 and copied flat at 127. `chain` is quadratic for both and for the same reason — every pivot's reach set is Θ(n) whichever arm picks it — and there `fbWork` is quadratic in calls **and** copies (1.99 / 2.00 over 2000 → 4000). **`lowlink` is neither extreme**: one DFS whatever the component structure, so every row reads 1.00–1.02 on `list` inside one trie depth, and 1.27–1.37 on the doubling that adds a level (500 → 1000 crosses 8³ = 512), which is the log₈ n term and not a quadratic. Its `nrOpUpdateValuesCopied` is flat at 127 on every shape it is read on here, because it performs no `//`. Where every reach is small the forward–backward arms win by a constant: on `fleet` at n = 2000 `fbNode` reads 524,466 calls against `lowlink`'s 1,171,109. **No figure here is a budget and none is offered as one**; they are filed with their derivation so the choice between arms is made against measurement rather than against intuition, and they are to be re-derived whenever either construction changes.
...
| `lowlink`                                       | none found to 100,000 on `chain`, `cycle`, `star` and `rand` (`ci/bench/partition-ceiling.sh`). The code contains no recursion, but its evaluation depth is not constant: it is set within a step by the trie's nesting, so it grows at most as Θ(log₈ n). Lowest passing `max-call-depth` on `cycle` under that instrument: 41 at 1,000 and 10,000, 43 at 100,000                                          | states no ceiling; refuses a target outside `nodes`, and a `nodes` entry that is not a string, by name                                                                                                                                                                   |
...
| `cycles`                                        | none found to **100,000** on `cycle`, `chain` and `rand` (`ci/bench/cost-classes.nix`, arm `cycles`); it inherits `lowlink`'s Θ(log₈ n) evaluation depth and adds no recursion of its own                                                                                                                                                                                                                   | states no ceiling **and the bound of the claim**                                                                                                                                                                                                                         |
| `cyclePaths`                                    | none found to **100,000** on `cycle`, `chain` and `rand` (arm `cyclePaths`). The witness search is iterated like `lowlink`, so it has no depth cap; it no longer inherits `pathsBetween`'s, under which it refused a 4,000-member ring                                                                                                                                                                      | states no ceiling **and the bound of the claim**                                                                                                                                                                                                                         |
...
**not** materialize the whole-graph `condensation`. This is longest-incoming-path
topological rank restricted to a dependent cone (cf. Tarjan 1983 — unheld, cite unverified).
...
★★ **THE ABSENCE OF A REFUSAL IS NOT EVIDENCE THAT YOUR `step` IS MONOTONE, and the gap is the same defect class.** Support is strictly weaker than foundedness: a conclusion drawn on an absence that later acquires a **circular** re-derivation is supported, and is returned. `p :- not r. p :- p. r :- a` has a non-monotone step (measured: `step {a}` yields `p`, `step {a,r}` does not), is **not** refused, and returns `p` — which the well-founded model makes FALSE. Closing that needs a well-founded engine, which is a later phase's work here — but **nothing need wait on it: `gen-scope` ships one today** (`wellFoundedModel`), so a caller needing foundedness runs it **one layer up**. It is unreachable from gen-graph by layering rather than by absence: gen-scope takes `gen-graph` as a flake input, so consuming it here would invert the dependency. The ceiling is instrumented, not just described: `test-seeded-circular-rederivation-is-supported-and-RETURNED`.

Measured price: one extra full step, and **the axis is ROUND COUNT, not size**. On the canonical closure instance over a 400-node chain (~399 rounds) it is **+0.73%** (45,307,630 → 45,637,203 thunks), `cpuTime` ranges overlapping either side — but on a **diameter-2 graph, which converges in 2 rounds, it is +30% to +37%**, stable at 402 and at 1,602 nodes. Shallow dependency graphs pay the high end.
...
| `ancestorsOf`                                       | Θ(depth): one closure element per ancestor, no per-step copy                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `foldPreorder` / `expandPreorder` / `foldReach`     | Θ(n + m) steps plus the caller's `expand` work; Θ(n log(n/32)) values copied into the visited set; peak memory grows with the values copied (`lib/preorder.nix`'s header; oracle `./ci/bench/walks-cost.sh`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `cycles`                                            | Θ((n + m) · log₈ n) through the `lowlink` arm, plus one accessor read per node for the self-loop test. `nrFunctionCalls` on `cycle` n = 1000 → 20000: 427,092 → 10,180,104 (span exponent 1.06), flat as calls / (n · ⌈log₈ n⌉) in 101.8–106.8; `nrOpUpdateValuesCopied` flat at 125. The per-node self-reachability it replaced was Θ(Σ_v \|reach⁺ v\|): 3,021,018 → 12,042,018 → 48,084,018 at 1000 → 2000 → 4000 (×3.99 per doubling)                                                                                                                                                                                                                                                                                                                                                                                               | read off the SCC partition (Tarjan 1972, Lemma 9): a node is cyclic iff its class has two or more members or it has a self-loop. Binds `lowlink` by name. Arm `cycles`; arm `cyclesUnhoisted` is the pre-hoist per-node construction                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `cyclePaths`                                        | Θ((n + m) · log₈ n) on every graph, cyclic or not: the `cycles` term above plus one iterated search per cyclic component, Θ((\|C\| + m_C) · log₈ \|C\|). `nrFunctionCalls` n = 1000 → 20000: `cycle` 612,962 → 14,679,943 (calls / (n · ⌈log₈ n⌉) in 146.8–153.2), `rand` 555,390 → 11,910,930 (119.1–151.8); `nrOpUpdateValuesCopied` flat at 125. The `pathsBetween` reconstruction it replaced copied Θ(L²), refused a 4,000-member ring at its depth cap, and was exponential on `rand` (47,958 calls at n = 25, 148,006,257 at n = 50)                                                                                                                                                                                                                                                                                            | the witness is the first simple path in `edges` order, found by a depth-first search with a global visited set restricted to the component — the path `pathsBetween` enumerates first. Arm `cyclePaths`; arm `cyclePathsUnhoisted` is the pre-hoist construction, `pathsBetween` included                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `cyclicEdgesWhere`                                  | Θ((n + m) · log₈ n) through the `lowlink` partition arm, plus one pass over the labelled edges. `nrFunctionCalls` with `p` accepting every edge, n = 1000 → 20000: `cycle` 451,095 → 10,660,107, `chain` 446,045 → 10,560,053, `rand` 550,845 → 12,863,196 — span exponents 1.06 / 1.06 / 1.05, ×2.00 per doubling inside one trie depth and ×2.36 on the doubling that adds a level; `nrOpUpdateValuesCopied` flat at 127. Bound to `fbNode` the `cycle` cells read 10,096,051 → 40,192,051 → 160,384,051 at n = 1000 / 2000 / 4000. `p` on one edge of a 5000-node `cycle`: 2,605,120 against `fbNode`'s 420,104; `p` never true: 50,021 on both (arms `cyclicEdgesWhere` / `…One` / `…None`)                                                                                                                                        | the `p` test short-circuits, so no `p`-edge forces no tag; the first tag read forces the whole partition. That trade — a constant factor on a sparse `p` for a class bound on every `p` — is the choice: `fbNode`'s per-endpoint cost depends on reach, so no threshold on the number of `p`-edges bounds it, and routing on that count inside the binder would be a second default beside the door's. A target outside `nodes` is refused by name                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `fbWork`                                            | linear on one large component on every axis — calls 1.00 and `nrOpUpdateValuesCopied` flat at 127 on `cycle`, n = 5000 → 20000. **Quadratic in copies on many components, whatever the node order**: its accumulator copies the whole tag map once per component, so copied reads 2.00 on `fleet` over the same n while calls read 1.03, and `sets` runs 1.35 → 1.53 on `fleet` (500 → 2000). **Quadratic in calls as well** where the backward walk re-covers the unassigned tail every round — `chain`, 1.99 on calls and 2.00 on copied over 2000 → 4000                                                                                                                                                                                                                                                                            | one forward–backward pass per COMPONENT over a `foldl'` accumulator. Complementary to `fbNode`, not ranked. ★ **It does NOT hoist its accessor, though it makes many closures**: hoisting builds a whole-graph `Θ(n)` memo up front, and this arm's closures **partition** the graph rather than re-covering it — each round walks a subgraph the earlier rounds have shrunk — so there is no second traversal over the same edges to spread that build cost over. Same mechanism as `dependentsOf`'s negative cell below. Measured 4.12× better on one deep `chain` (the one shape whose backward walk re-covers the whole unassigned tail every round) and 1.003–1.25× **worse** on `complete`, `fleet`, `wide`, `total` and `cycle`, the decisive losses being `cycle` (1.243×) and `complete`'s `nrLookups` (1.397×). ★ **Restriction ordering is not the lever**, measured: an arm memoizing plain adjacency and restricting *before* wrapping recovers none of it — worst of three on `fleet`, and within 4 sets of the full hoist on `cycle` while both memoizing arms sit ~2,400 above the shipped one. Arms `fbWork` / `fbWorkHoisted` |
| `condensation` / `lowlink`                          | Θ((n + m) · log₈ n) on every shape, the log being its persistent trie's. `nrFunctionCalls` n = 1000 → 20000: `cycle` 433,180 → 10,300,192, `chain` 548,039 → 12,600,049, `rand` 508,180 → 12,320,786 — span exponents 1.06 / 1.05 / 1.06, and flat as calls / (n · ⌈log₈ n⌉): 108 at n = 1000–4000 and 103 at 8000–20000 on `cycle`. The finisher's domain check is in these figures, about 7n calls. `list.elements` span exponents 1.07 / 1.07 / 1.08. `nrOpUpdateValuesCopied` flat at 125 on `cycle` and `chain`; on `rand` it rises with the component count (125 → 2,175), and that is the shared finisher's term, not the arm's: `fbNode` reads the identical 1,053 and 2,139 at n = 2000 and 4000                                                                                                                              | one `genericClosure` loop, one step per push, edge, finished frame or popped member; every record the loop carries is built by a constructor that forces each field                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `topoOrder` / `phaseOrder`                          | O(n + E) decrements through the Kahn arm, and **near-linear in allocation** — exponent 1.07–1.08 on `list.elements` and 0.99–1.12 on `sets.elements`, all four acyclic shapes, read on arm `topoOrderKahn`. ★ Through the DOOR a shape the certificate admits is cheaper by a class of construction rather than a constant — `18n` on a chain against the loop's 1.07 exponent, 2.010 `list`/edge against 10.042 on the dense total order — and a shape it refuses is the loop plus `2n − 1`. The `ok = false` cycle report runs the `lowlink` arm twice (once through `cycles`, once for `sccOf`), Θ((n + m) · log₈ n): on `cycle` 889,195 → 21,060,219 `nrFunctionCalls` at n = 1000 → 20000, calls / (n · ⌈log₈ n⌉) in 210.6–222.3; the per-node guard it replaced read 3,476,115 → 12,952,115 → 49,904,115 at 1000 → 2000 → 4000   | nothing the loop carries is quadratic: the emitted sequence is a binary-counter run list (Θ(n log n)), the indegree map is a residue over a rebuilt base, and the ready set is a leftist heap (Θ(n log n)), where a re-sorted array was a third quadratic worth 60% of the `wide` list allocation. The heap's `sets.elements` price, 63 → 89 attrsets per node over n = 1000 → 8000, is now the leading set-axis term. No frame ceiling — the loop is a bounded iteration, so no node count aborts or is refused                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `seededFixpoint`                                    | O(work per delta), **plus one full step at convergence**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | semi-naive: each iteration touches only the frontier. The support check re-derives once from the converged accumulator, and **what dilutes it is ROUND COUNT, not size** — the loop's per-round unions and differences dominate only when there are many rounds. Deep instance, 400-node chain at ~399 rounds: **45,307,630 → 45,637,203** thunks, **+0.73%**; `cpuTime` 25.3–29.2s against 28.2–30.2s over three runs a side, which OVERLAPS, so the thunk count resolves it and the wall clock does not. **Shallow instance, diameter-2 at 2 rounds: +30% to +37%**, and the ratio holds across a 4× size change (402 and 1,602 nodes), which is what identifies the axis. A caller on a shallow dependency graph pays the high end                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| "Which loop, in order, for a message?"    | `cyclePaths` (Θ((n + m) · log₈ n))                                                                                       | hand-rolled DFS per node (enumerates every simple path even when acyclic) |
