{ lib, graphLib, ... }:
let
  inherit (graphLib) fixpoint compose transitiveClosure transitiveReduction materialize;
...
in
{
  fixpoint-tests = {
    test-closure-chain = {
      ...
      in builtins.sort builtins.lessThan (closure."a" or []);
      expected = [ "b" "c" "d" ];
    };
    ...
    test-reduction-chain-unchanged = {
      ...
      in builtins.sort builtins.lessThan (red."a" or []);
      expected = [ "b" ];
    };
    ...
    test-compose-basic = {
      ...
      in builtins.sort builtins.lessThan (comp."a" or []);
      expected = [ "c" ];
    };
    test-compose-leaf = {
      expr = let
    ...
    test-fixpoint-converges = {
      expr = let
        result = fixpoint {
          seed = { a = []; };
          step = current:
            if builtins.length (current.a or []) < 3
            then current // { a = (current.a or []) ++ ["x"]; }
            else current;
        };
      in builtins.length (result.a or []);
      expected = 3;
    };
    test-fixpoint-monotonicity-violation = {
      expr = !(builtins.tryEval (fixpoint {
        seed = { a = ["x" "y"]; };
        step = _: { a = ["x"]; };
      })).success;
      expected = true;
